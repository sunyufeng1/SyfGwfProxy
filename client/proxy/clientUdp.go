...
func (this *ClientObj) initListenerUDP() {
	...
	if syf.CheckErrorResult(err, "read ServerIP ") {
		return
	}

	//读取的数据为json格式，需要进行解码
	err = json.Unmarshal(data, jsonStr)
	if syf.CheckErrorResult(err, "Unmarshal ServerIP ") {
		return
	}
	...
	for {
		this.handleUDPRequest(clientConn, jsonStr.Ip)
	}
}
func (this *ClientObj) handleUDPRequest(clientConn *net.UDPConn, Ip string) {
	if clientConn == nil {
		return
	}
	defer clientConn.Close()
	...
	proxyServer, err := net.DialUDP("udp", nil, udpAddr)
	if err != nil {
	...
	b := make([]byte, 1024)
	num, addr, err := proxyServer.ReadFromUDP(b) //第一次从代理服务器中读取
